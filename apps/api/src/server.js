 ...
 *
 * Démarrage local :  copier .env.example vers .env, renseigner le jeton, puis
 *   pnpm --filter api dev
 * Déployable tel quel sur tout hébergeur Node (Render, Railway, VPS…).
 */
...
if (!PROJECT_ID || !TOKEN) {
  console.error('SANITY_PROJECT_ID et SANITY_API_WRITE_TOKEN sont requis (voir .env.example).');
  process.exit(1);
}
...
server.listen(PORT, () => {
  console.log(`API HGWF Cargo sur http://localhost:${PORT} (dataset ${DATASET})`);
});
